...
def entity_detection(text):
    ...
    ent_reponse = client.detect_entities(Text=text, LanguageCode = code)
    entities = ent_reponse['Entities']
    if not entities:
        print('There is no entity in the %s text'%language)
        return
    print("The entities in the %s text are:" % language)
    for ent in entities:
       print(ent['Text'] + ' is ' + ent['Type'])
    print()
        
entity_detection(english)
entity_detection(spanish)
entity_detection(french)
entity_detection(italian)
